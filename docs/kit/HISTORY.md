...
Design-decision log for the kit. Newest first. The *why* behind each change (ADR-style).
Each entry was processed through kit-improve and approved by the user.

> **Versioning lives in [CHANGELOG.md](../../CHANGELOG.md)** — auto-generated by release-please from Conventional Commits. This file is NOT the version source. Entries below predate the split and keep their original `vX` labels as historical record; **new entries use a dated heading** (`## YYYY-MM-DD — title`) with no version number.
